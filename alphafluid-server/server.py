...
def send_bought(st):
	try:
		urllib2.urlopen("http://ora2.tutschonwieder.net:8080/apex/tensai-prod/put/sellproduct/"+apikey+"/1/" + str(mapping[int(st)]) + "/1")
	except:
		log("!!!!!!!!!!!!!!!!!!!!!! LICK DOWN !!!!!!!!!!!!!!!!!!!!!")

def send_empty(st):
	try:
		urllib2.urlopen("http://ora2.tutschonwieder.net:8080/apex/tensai-prod/put/schachtleer/"+apikey+"/1/" + str(mapping[int(st)]))
	except:
		log("!!!!!!!!!!!!!!!!!!!!!! LICK DOWN !!!!!!!!!!!!!!!!!!!!!")

def lick_get_level(shaft):
	try:
		lines = urllib2.urlopen("http://ora2.tutschonwieder.net:8080/apex/tensai-prod/get/fuellstand/1/"+str(shaft)).readlines()
		for line in lines:
			print line
